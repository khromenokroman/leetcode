...
remaining elements of nums are not important as well as the size of nums. Return k. Custom Judge:
   ...
   public:
    int removeElement(std::vector<int>& nums, int val) {
        ...
        for (auto const& item : nums) {
            if (item != val) {
                tmp_vec.emplace_back(item);
            }
        }
        nums = std::move(tmp_vec);
        return nums.size();
    }
};
...
int main() {
    ...
    auto answer = sol.removeElement(vec, 3);
