   ...
   public:
    char nextGreatestLetter(std::vector<char>& letters, char target) {
        for (auto item : letters) {
            if (static_cast<int>(item) > static_cast<int>(target)) {
                return item;
            }
        }
        return letters[0];
...
int main() {
    Solution sol;
    //    std::vector<char> vec{'c', 'f', 'j'};
    //    std::cout << sol.nextGreatestLetter(vec, 'a') << std::endl;
    //    std::vector<char> vec{'c', 'f', 'j'};
    //    std::cout << sol.nextGreatestLetter(vec, 'c') << std::endl;
    std::vector<char> vec{'x', 'x', 'y', 'y'};
    std::cout << sol.nextGreatestLetter(vec, 'z') << std::endl;
    return 0;
}