...
in nums initially. The remaining elements of nums are not important as well as the size of nums. Return k. Custom Judge:
...
class Solution {
   public:
    int removeDuplicates(std::vector<int>& nums) {
        std::map<int, int> dict_num;
        for (auto i = 0; i < nums.size(); i++) {
            dict_num[nums[i]]++;
        }
        int count = 0;
        for (auto const& item : dict_num) {
            nums[count] = item.first;
            count++;
        }
        return static_cast<int>(dict_num.size());
    }
};

int main() {
    //    std::vector<int> vec{1, 1, 2};
    std::vector<int> vec{-3, -1, 0, 0, 0, 3, 3};

    Solution sol;
    int answer = sol.removeDuplicates(vec);
